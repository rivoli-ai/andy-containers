...
- **andy-cli is a hard dependency.** A container that ships without `andy-cli` on `$PATH` cannot host a Headless run; provisioning succeeds but the runner exits non-zero. Mitigated by the `EnvironmentProfile.BaseImageRef` contract (X4): the image bound to a `HeadlessContainer` profile is expected to ship andy-cli. Since rivoli-ai/andy-tasks#390 the `andy-cli-dev` template provisions from the pre-baked `andy-agent-cli:latest` image (built locally from `images/agent-cli/Dockerfile`, warmed at startup by `AgentCliImageWarmer`), so andy-cli is on `$PATH` at image level and containers reach Ready in seconds; a plain-base fallback (non-Docker providers) still source-builds it in `post_create`. Future work: pre-flight check before spawn.
