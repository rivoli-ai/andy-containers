...
- **andy-cli is a hard dependency.** A container that ships without `andy-cli` on `$PATH` cannot host a Headless run; provisioning succeeds but the runner exits non-zero. Mitigated by the `EnvironmentProfile.BaseImageRef` contract (X4): the image bound to a `HeadlessContainer` profile is expected to ship andy-cli. Since rivoli-ai/andy-tasks#390 the `andy-cli-dev` template provisions from a locally built, revision-tagged `andy-agent-cli` image (built from `images/agent-cli/Dockerfile`, warmed at startup by `AgentCliImageWarmer`, and pinned to the `LocalImages.AgentCliGitRevision` commit), so andy-cli is on `$PATH` at image level and containers reach Ready in seconds. Bumping that revision creates a new tag, so upgrades converge without manually deleting the old image and rollback selects the prior release/tag. A plain-base fallback (non-Docker providers) still source-builds it in `post_create`. Future work: pre-flight check before spawn.
