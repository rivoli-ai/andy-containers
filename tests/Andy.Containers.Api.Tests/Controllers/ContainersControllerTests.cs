...
public class ContainersControllerTests : IDisposable
{
    ...
    [Fact]
    public async Task Get_LegacyEmailOwner_WithCanonicalSubject_Returns200()
    {
        // A refreshed human token uses its stable subject id, while legacy
        // and OBO-created containers may still be stamped with that same
        // principal's verified email address. Both claims identify the current
        // caller, so this is ownership—not cross-user access.
        _mockCurrentUser.Setup(u => u.IsAdmin()).Returns(false);
        _mockCurrentUser.Setup(u => u.IsServiceAccount()).Returns(false);
        _mockCurrentUser.Setup(u => u.GetUserId()).Returns("canonical-subject-id");
        _mockCurrentUser.Setup(u => u.GetEmail()).Returns("test@andy.local");
        var container = new Container
        {
            Id = Guid.NewGuid(),
            Name = "legacy-goal-container",
            OwnerId = "TEST@ANDY.LOCAL",
        };
        ArrangeGet(container);
