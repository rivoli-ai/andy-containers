...
// Runtime identity is injected by HeadlessRunner, never serialized here.
public class RunConfiguratorTests
{
    ...
    [Fact]
    public async Task ConfigureAsync_PreservesOrdinaryAgentEnvVars_WithoutAddingRuntimeIdentity()
    {
        var run = SeedRun();
        var spec = TriageAgent() with
        {
            EnvVars = new Dictionary<string, string> { ["ORDINARY"] = "kept" },
        };
        ...
        var envVars = captured!.EnvVars!;
        envVars.Should().ContainSingle().Which
            .Should().Be(new KeyValuePair<string, string>("ORDINARY", "kept"));
        envVars.Keys.Should().NotContain(name => EnvVarNames.IsRuntimeIdentity(name));
    }

    [Theory]
    [InlineData(EnvVarNames.AndyToken)]
    [InlineData(EnvVarNames.AndyProxyUrl)]
    [InlineData(EnvVarNames.AndyMcpUrl)]
    public async Task ConfigureAsync_ReservedRuntimeIdentity_IsStripped(string reservedName)
    {
        var run = SeedRun();
        var spec = TriageAgent() with
        {
            ...
            {
                [reservedName] = "agent-authored-value",
            },
        };
        _agents.Setup(a => a.GetAgentAsync(run.AgentId, run.AgentRevision, It.IsAny<CancellationToken>()))
            .ReturnsAsync(spec);
        var config = SampleConfig(run.Id);
        _builder.Setup(b => b.Build(It.Is<Run>(r => r.Id == run.Id),
            It.IsAny<AgentSpec>())).Returns(config);
        _writer.Setup(w => w.WriteAsync(config, It.IsAny<CancellationToken>()))
            .ReturnsAsync("/tmp/runs/x/config.json");
        ...
        result.IsSuccess.Should().BeTrue();
        _builder.Verify(b => b.Build(It.Is<Run>(r => r.Id == run.Id),
            It.Is<AgentSpec>(a => a.EnvVars != null && !a.EnvVars.ContainsKey(reservedName))));
        _writer.Verify(w => w.WriteAsync(It.IsAny<HeadlessRunConfig>(), It.IsAny<CancellationToken>()));
    }
