...
public class ContainerOrchestrationService : IContainerService
{
    ...
    public async Task<Container> CreateContainerAsync(CreateContainerRequest request, CancellationToken ct)
    {
        ...
        // `andy-agent-cli:latest` is built locally by the DOCKER provider
        ...
        // (andy-desktop-*, andy-agent-cli:latest — see LocalImages)
    ...
    /// pre-baked <c>andy-agent-cli:latest</c> image only exists on providers
